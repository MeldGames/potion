...
pub fn spring_slot(
    ...
    for (slot_entity, mut slot, slot_settings, _grace_period) in &mut slots {
        if let Some(particle_entity) = slot.containing {
            ...
            let strength = slot_settings.0.strength;

            let impulse = {
                let displacement = particle_a
                    .translation()
                    .instant(&particle_b.translation())
                    .displacement;
                let displacement_dir = displacement.normalize_or_zero();

                let mut impulse = strength * displacement_dir;

                // Cap out the angular impulse to not be greater than the displacement so we don't constantly overshoot.
                // TODO: simplify this
                if (displacement.x > 0.0 && impulse.x > displacement.x)
                    || (displacement.x < 0.0 && impulse.x < displacement.x)
                {
                    impulse.x = displacement.x;
                }

                if (displacement.y > 0.0 && impulse.y > displacement.y)
                    || (displacement.y < 0.0 && impulse.y < displacement.y)
                {
                    impulse.y = displacement.y;
                }

                if (displacement.z > 0.0 && impulse.z > displacement.z)
                    || (displacement.z < 0.0 && impulse.z < displacement.z)
                {
                    impulse.z = displacement.z;
                }
            ...
            if let Some(mut slot_impulse) = slot_impulse {
                ...
                slot_impulse.torque_impulse += angular_impulse;
